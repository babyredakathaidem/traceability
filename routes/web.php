...
Route::middleware(['auth', 'verified', 'tenant.ready', 'tenant'])->group(function () {
    ...
    Route::get('/batches/transform',  [BatchTransformationController::class, 'show'])  ->name('batches.transform.show');
    Route::post('/batches/transform', [BatchTransformationController::class, 'store']) ->name('batches.transform.store');
    ...
    // ══ TRANSFER EVENTS ════════════════════════════════════════════════════
    // Create pages
    Route::get('/events/create/transfer-in', function () {
        $locations = \App\Models\TraceLocation::where('enterprise_id', auth()->user()->enterprise_id)
            ->where('status', 'active')
            ->get(['id', 'name', 'gln', 'address_detail', 'province', 'ai_type']);
        $products = \App\Models\Product::where('enterprise_id', auth()->user()->enterprise_id)
            ->where('status', 'active')
    ...
    // Giữ để không phải sửa Vue component, trỏ thẳng vào event-centric routes
