  ...
  <div class="space-y-8 max-w-6xl mx-auto pb-12">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6" data-aos="fade-down">
      <div class="space-y-1">
        ...
        <h1 class="text-3xl font-black text-white tracking-tight">Hồ sơ năng lực & <span class="text-brand-400">Chứng
            nhận</span></h1>
        <p class="text-white/40 text-sm font-medium">Quản lý các tiêu chuẩn VietGAP, GlobalGAP, ISO để gia tăng giá trị
          thương hiệu.</p>
      </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4" data-aos="fade-up" data-aos-delay="100">
      <div v-for="(stat, i) in [
        ...
        <component :is="stat.icon"
          class="absolute -right-4 -bottom-4 w-24 h-24 opacity-5 group-hover:opacity-10 transition-all duration-500" />
    ...
    <div v-if="certificates.length === 0"
      ...
      data-aos-delay="200">
      <div
        class="w-24 h-24 rounded-full bg-brand-500/10 flex items-center justify-center mx-auto mb-6 shadow-2xl shadow-brand-500/10 animate-pulse">
        <AcademicCapIcon class="w-12 h-12 text-brand-500/40" />
      </div>
      <h3 class="text-xl font-bold text-white/80">Chưa có chứng nhận nào</h3>
      <p class="text-white/30 text-sm mt-2 max-w-xs mx-auto leading-relaxed">Hãy bắt đầu bằng việc khai báo các chứng
        chỉ chất lượng để khách hàng tin tưởng hơn.</p>
      <button @click="openCreateModal"
        class="mt-6 text-brand-400 font-bold text-sm hover:underline uppercase tracking-widest">Khai báo ngay →</button>
    </div>

    <div v-else class="grid grid-cols-1 lg:grid-cols-2 gap-6" v-auto-animate>
      <div v-for="(cert, i) in certificates" :key="cert.id"
        ...
        <!-- Background Glow -->
        <div
          class="absolute -right-10 -top-10 w-40 h-40 bg-brand-500/5 blur-3xl rounded-full group-hover:bg-brand-500/10 transition-all duration-700">
        </div>
        ...
        <div class="flex items-start justify-between gap-4 relative z-10">
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-3 mb-2">
              <div
                class="w-8 h-8 rounded-lg bg-brand-500/20 flex items-center justify-center text-brand-400 shadow-inner">
                <ClipboardDocumentCheckIcon class="w-5 h-5" />
              </div>
              <h3
                class="text-lg font-black text-white truncate leading-tight group-hover:text-brand-300 transition-colors">
                {{ cert.name }}</h3>
            </div>
          ...
          <div class="flex flex-col gap-2 shrink-0">
            <span :class="statusClass(cert.status, cert.is_expired)"
              class="text-[9px] px-2 py-1 rounded-lg border font-black uppercase tracking-widest text-center">
              {{ statusLabel(cert.status, cert.is_expired) }}
            </span>
            <div class="flex gap-1 mt-auto pt-12">
              <button @click="openEditModal(cert)"
                class="p-2.5 rounded-xl bg-white/5 border border-white/10 text-white/30 hover:text-white hover:bg-white/10 transition-all"
                title="Sửa">
                <PencilSquareIcon class="w-5 h-5" />
              </button>
              <button @click="remove(cert.id)"
                class="p-2.5 rounded-xl bg-white/5 border border-white/10 text-white/20 hover:text-red-400 hover:bg-red-500/10 transition-all"
                title="Xóa">
        ...
        <div class="mt-6 pt-4 border-t border-white/5 flex items-center justify-between relative z-10">
          <div class="flex items-center gap-2">
            ...
            <span class="text-[10px] text-brand-400/60 font-black uppercase tracking-widest italic">Áp dụng cho {{
              cert.batches_count }} lô hàng</span>
          </div>
          <button v-if="cert.image_url" @click="lightboxUrl = cert.image_url"
            class="text-[10px] font-black text-white/40 hover:text-white transition-colors flex items-center gap-1.5">
    ...
    <div class="bg-cosmic-950 p-8 relative overflow-hidden">
      ...
      <div class="relative z-10">
        <div class="flex items-center gap-4 mb-8">
          <div
            class="w-12 h-12 rounded-2xl bg-brand-500/20 border border-brand-500/30 flex items-center justify-center text-brand-400 shadow-xl shadow-brand-900/20">
        ...
        <form @submit.prevent="submit" class="space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
              <div class="flex items-center justify-between mb-2">
                <label class="text-[10px] text-white/30 uppercase font-black tracking-[0.2em]">Tên chứng chỉ / Tiêu
                  chuẩn</label>
              ...
              <div class="relative group">
                <IdentificationIcon
                  class="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-white/20 group-focus-within:text-brand-400 transition-colors" />
                <input v-model="form.name" list="std-names" :class="[inputCls, 'pl-12']"
                  placeholder="Gõ tên hoặc chọn gợi ý..." required />
            ...
            <!-- Organization -->
            <div>
              <label class="text-[10px] text-white/30 uppercase font-black tracking-[0.2em] block mb-2">Tổ chức
                cấp</label>
              <div class="relative group">
                <BuildingOfficeIcon
                  class="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-white/20 group-focus-within:text-brand-400 transition-colors" />
            ...
            <!-- Certificate No -->
            <div>
              <label class="text-[10px] text-white/30 uppercase font-black tracking-[0.2em] block mb-2">Số hiệu văn
                bản</label>
              <div class="relative group">
                <GlobeAltIcon
                  class="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-white/20 group-focus-within:text-brand-400 transition-colors" />
            ...
            <!-- Scope -->
            <div class="md:col-span-2">
              <label class="text-[10px] text-white/30 uppercase font-black tracking-[0.2em] block mb-2">Phạm vi chứng
                nhận</label>
              <textarea v-model="form.scope" rows="2" :class="inputCls"
                placeholder="Ví dụ: Áp dụng cho toàn bộ quy trình sản xuất lúa gạo tại trang trại X..."></textarea>
            </div>
            ...
            <div>
              <label class="text-[10px] text-white/30 uppercase font-black tracking-[0.2em] block mb-2">Ngày hết
                hạn</label>
            ...
            <!-- Image -->
            <div class="md:col-span-2">
              <label class="text-[10px] text-white/30 uppercase font-black tracking-[0.2em] block mb-2">Ảnh chụp / Bản
                scan minh chứng</label>
              <div
                class="relative border-2 border-dashed border-white/5 rounded-2xl p-4 hover:border-brand-500/30 transition-all bg-black/20 group">
                <input type="file" @input="form.image = $event.target.files[0]"
                  class="absolute inset-0 opacity-0 cursor-pointer z-20" />
                <div class="flex items-center gap-4">
                  <div
                    class="w-12 h-12 rounded-xl bg-white/5 flex items-center justify-center text-white/20 group-hover:text-brand-400 transition-colors">
                    <DocumentArrowUpIcon class="w-6 h-6" />
                  </div>
                  <div>
                    <div class="text-xs font-bold text-white/60">Kéo thả hoặc Click để tải lên</div>
                    <div class="text-[10px] text-white/20 uppercase font-black mt-0.5">Hỗ trợ: JPG, PNG, PDF (Max 5MB)
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div class="mt-8 pt-6 border-t border-white/5 flex justify-end gap-4">
            <button type="button" @click="closeModal"
              class="px-6 py-3 rounded-2xl text-white/40 hover:text-white text-xs font-black uppercase tracking-widest transition-colors">
              Hủy bỏ
            </button>
